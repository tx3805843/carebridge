...
- [x] Supabase project provisioned (EU region — see brief Part 1.1 for rationale) — `carebridge`, ref `rqnyleqluzblupfnsfmt`, region `eu-west-2` (London, per ADR-0003), `https://rqnyleqluzblupfnsfmt.supabase.co`, Hobby tier ($0/mo). Empty — migrations not yet pushed, CI deploy secrets not yet wired.
- [x] CI green (lint, typecheck, test) on `origin/main` — `deploy` still fails (expected: no hosted Supabase project/secrets yet); `migration-dry-run` only triggers on PRs touching `supabase/migrations/**`, never run against a real PR yet
...
- [ ] Migrations have not been pushed to the new hosted `carebridge` project, and `SUPABASE_ACCESS_TOKEN`/`SUPABASE_PROJECT_REF` secrets are not yet set in GitHub, so `deploy.yml` and `migration-dry-run.yml` still can't succeed against it. Needs: `supabase link --project-ref rqnyleqluzblupfnsfmt`, `supabase db push`, then set the two repo secrets.
